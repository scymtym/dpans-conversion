...
(defmacro indenting-output ((stream indent transform) &body body)
  (check-type stream symbol)
  (a:with-unique-names (old-left-margin new-left-margin extra-indent)
    (a:once-only (transform)
      `(let ((,old-left-margin (clim:bounding-rectangle-min-x
                                (clime:stream-page-region ,stream))))
         (clim:indenting-output (,stream ,indent)
           (let* ((,new-left-margin (clim:bounding-rectangle-min-x
                                     (clime:stream-page-region ,stream)))
                  (,extra-indent    (- ,new-left-margin ,old-left-margin)))
  ...
  (flet ((draw-it (stream)
    ...
    (cond ((eql (depth transform) 1)
          ...
          (t
           ...
           (unwind-protect
                (indenting-output (stream '(2 :character) transform)
                  (recurse '(:element . *)))
      ...
      (clim:formatting-cell (stream)
        (clime:with-temporary-margins (stream :right `(:absolute ,(line-width transform)))
          (let ((bullet (format nil "~D. " (1+ index))))
            ...
            (indenting-output (stream indent transform)
              (recurse)))
          (incf (second context)))))))
...
(define-display (:enumeration-list)
  ...
  (let* ((items  (bp:node-relation builder '((:element . *)) node))
         (indent (format nil "~D. " (1+ (length items)))))
...
(define-display (:list-item)
  (clim:formatting-cell (stream)
    (clime:with-temporary-margins (stream :right `(:absolute ,(line-width transform)))
      (let ((bullet  "• "))
        (write-string bullet stream)
        (indenting-output (stream bullet transform)
          (recurse))))))
